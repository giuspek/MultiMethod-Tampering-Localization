function [] = get_map(image_name)
   ...
   % ADQ2 ANALYSIS
    try
        if isTIF
           OutputADQ = analyze_adq2(image_name_jpg); 
        else
           OutputADQ = analyze_adq2(image_name); 
        end   
        ...
        if sum(sum(decision_ss2)) >= 2350 && sum(sum(decision_ss2)) <= 44650
            ...
            imwrite(map, strcat('DEMO-RESULTS/',image_name_bmp));
            return
        end
    catch e
         disp('Error on ADQ2');
    ...
    try
        ...
        elseif abs(max(max(map)) - min(min(map))) > 7
            result = map < 0; 
        else
            result = map < -1000000;
        end
        ...
        if sum(sum(decision_ss2)) >= 772 && sum(sum(decision_ss2)) <= 14671
            disp(['CFA']);
            decision_ss2 = imresize(decision_ss2, [1500 2000]);
            imwrite(decision_ss2,strcat('DEMO-RESULTS/', image_name_bmp));
            return
        end
    catch e
        disp('Error on CFA');
    ...
    try
        ...
        OutputMap = analyze_noi4(image_name);
        ...
        if sum(sum(imggez))<100000 || sum(sum(imggez))>2850000 
            ...
            if sum(sum(imggez))>=100000 && sum(sum(imggez))<=2850000
                disp(['NOI4 4']);
                imwrite(imggez,strcat('DEMO-RESULTS/', image_name_bmp));
                return
            end
        else
            disp(['NOI4 8']);
    ...
    try
        ...
        if sum(sum(fusion_labeling)) >= 150000 && sum(sum(fusion_labeling)) <= 2850000 
            disp(['PRNU']);
            imwrite(fusion_labeling, strcat('DEMO-RESULTS/',image_name_bmp))
            return
        end
    catch e
        disp('Error on PRNU');
    ...
    imwrite(meta_sugiu,strcat('DEMO-RESULTS/', image_name_bmp));
end
